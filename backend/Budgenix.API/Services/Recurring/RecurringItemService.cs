...
namespace Budgenix.Services.Recurring
{
    public class RecurringItemService
    {
        ...
        public DateTime? GetNextOccurrenceDate(RecurringItem item, DateTime fromDate)
        {
            ...
            var next = item.StartDate.Date;

            // Keep advancing until next >= fromDate
            while (next < fromDate)
            {
                next = item.Frequency switch
                {
                    RecurrenceTypeEnum.Daily => next.AddDays(1),
                    RecurrenceTypeEnum.Weekly => next.AddDays(7),
                    RecurrenceTypeEnum.BiWeekly => next.AddDays(14),
                    RecurrenceTypeEnum.Monthly => next.AddMonths(1),
                    RecurrenceTypeEnum.Quarterly => next.AddMonths(3),
                    RecurrenceTypeEnum.Yearly => next.AddYears(1),
                    _ => next
                };
            }

            // If next is beyond EndDate, it's no longer active
            if (item.EndDate != null && next > item.EndDate)
                return null;
