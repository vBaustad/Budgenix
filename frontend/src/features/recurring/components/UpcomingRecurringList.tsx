import { useCurrency } from '@/context/CurrencyContext';
import { RecurringItemDto } from '@/types/finance/recurring';
import { RecurrenceFrequencyLabels } from '@/types/shared/recurrence';
import { formatCurrency, formatDate } from '@/utils/formatting';
import { AppIcons } from '@/components/icons/AppIcons';
import {
  useDeleteRecurringItem,
  useSkipRecurringItem,
  useTriggerRecurringItem,
} from '../services/recurringService';

type Props = {
  recurringItems: RecurringItemDto[];
  loading: boolean;
  onSelect?: (item: RecurringItemDto) => void;
};

const MAX_UPCOMING = 3;

export default function UpcomingRecurringItemsList({ recurringItems, loading, onSelect }: Props) {
  const { currency: userCurrency } = useCurrency();
  const { mutate: deleteRecurring } = useDeleteRecurringItem();
  const { mutate: skipRecurring } = useSkipRecurringItem();
  const { mutate: triggerRecurring } = useTriggerRecurringItem();

  if (loading) {
    return <span className="loading loading-spinner loading-md" />;
  }

  if (recurringItems.length === 0) {
    return <p className="text-base-content">No upcoming recurring items found.</p>;
  }

  const sorted = [...recurringItems]
    .filter((item) => item.nextOccurrenceDate)
    .sort((a, b) => new Date(a.nextOccurrenceDate!).getTime() - new Date(b.nextOccurrenceDate!).getTime())
    .slice(0, MAX_UPCOMING);

  return (
    <ul className="space-y-4">
      {sorted.map((item) => (
        <li
          key={item.id}
          className="relative bg-base-100 rounded-xl p-2 border border-base-300 shadow-md w-full"
        >
          <div className="flex justify-between items-center flex-wrap gap-1">
            <div className="flex flex-wrap items-center gap-1 min-w-0">
              <div className="font-medium text-base-content truncate">{item.name}</div>
              <div className="flex items-center gap-1 text-sm text-base-content">
                <AppIcons.calendar className="w-4 h-4" />
                {formatDate(item.nextOccurrenceDate)} · {RecurrenceFrequencyLabels[item.frequency]}
              </div>
              {item.categoryName && (
                <div className="badge badge-outline badge-sm text-xs">
                  {item.categoryName}
                </div>
              )}
            </div>

            <div className="text-sm font-semibold text-base-content">
              {formatCurrency(item.amount, userCurrency)}
            </div>
          </div>

          <div className="flex gap-2 mt-3 flex-wrap">
            <button
              className="btn btn-xs btn-success"
              onClick={() => triggerRecurring(item.id)}
            >
              <AppIcons.complete className="w-4 h-4" />
              Mark Paid
            </button>
            <button
              className="btn btn-xs btn-info"
              onClick={() => skipRecurring({ id: item.id })}
            >
              <AppIcons.recurring className="w-4 h-4" />
              Skip
            </button>
            <button
              className="btn btn-xs btn-warning"
              onClick={() => onSelect?.(item)}
            >
              <AppIcons.edit className="w-4 h-4" />
              Edit
            </button>
          </div>

          <button
            className="absolute bottom-2 right-2 p-1 rounded-md hover:bg-error/20 transition"
            onClick={() => deleteRecurring(item.id)}
            aria-label="Delete"
            title="Delete"
          >
            <AppIcons.delete className="w-4 h-4 text-error hover:text-error-content" />
          </button>
        </li>
      ))}
    </ul>
  );
}
